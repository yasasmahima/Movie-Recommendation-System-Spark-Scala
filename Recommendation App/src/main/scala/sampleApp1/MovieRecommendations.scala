...
// Method to make CosineSimilarityMatrix
...
//  Method to map names to each movie ID
  def mapMovieIdAndName():Map[Int,String]={
    ...
    val idAndNameMapped: Map[Int, String] = Source.fromFile("D:\\Zone24x7\\Recommendation App\\src\\main\\resources\\u.item").getLines().map(line =>{
...
//  Def method to get top 10 related movie to given
  def suggestTop10Movies(moviesAndSimilarityScore: RDD[((Int, Int), (Double, Int))],args:Array[String]): Unit = {
    ...
    val scoreThreshold: Double = 0.97  //Assumed value
    val coOccurenceThreshold: Double = 50.0 //Assumed value
...
//    Ger Similarity Score for each movie pairs using CosineSimilarity Matrix
    ...
    if(args.length>0)
      ...
      suggestTop10Movies(cosineSimilarityScoreForEachPair,args)

    else
      println("Please give movie Id as command line argument")
